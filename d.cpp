...
bool GeoMipMapping::initHeightMap(const char*heightFile)
{
	...
	if (png_image_begin_read_from_file(&image_png, heightFile))
	{
		...
		if (png_image_finish_read(&image_png, NULL/*background*/, buffer_png, 0/*row_stride*/, nullptr/*colormap*/))
		{
			...
			{
				for (unsigned int y = 0; y < mCY; y++)
				{
					unsigned short * src = (unsigned short*)mData + y * mCX;
					for (unsigned int x = 0; x < mCX; x++)
					{
						float tmp = (float)(src[x]) * (0.1f / 40.0f);
						heightData_.heightData_[(mCY - y - 1) * mCX + x] = (float)(src[x]) * (0.1f / 40.0f);
					}
				}
			}
...
bool GeoMipMapping::initHeightTexture(unsigned int xsize, unsigned int ysize)
{
	...
	texture_->target_ = GL_TEXTURE_CUBE_MAP;
...
Shader* GeoMipMapping::initShader()
{
	...
	char fragShader[] = "#version 330 core \n"
		...
		"void main()"
		"{"
		"color =  texture(hightmap, texCoord);"//texture(hightmap, texCoord);
		"}";
...
#endif
			vertexData[num_vertexs_ - 1].TexCoords = V2f(((float)x) / xsize_, (float(ysize_ - y))/ ysize_);
		}
	}
